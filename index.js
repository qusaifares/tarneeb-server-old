...
class Room {
  ...
  selectBid = (playerNumber, bid) => {
    ...
    // If everyone passes without bidding, shuffle and restart bid
    if (this.bidders.length === 0 && this.bid === 0) {
      for (let i = 1; i <= 4; i++) {
        this.players[`player${i}`].bid = 0;
      }
      this.bidders = [1, 2, 3, 4];
      this.startBid();
      return;
    ...
    if (this.bid === 0 || (this.bidders.length > 1 && this.bid !== 13)) {
      ...
      io.in(this.name).emit('end_bid', {
        highestBidder: this.winningBidder,
        highestBid: this.bid,
        room: this
      });
    }
